 ...
 * the License.
 */
package com.zhongxun.commons.generator.config.builder;
...
import com.zhongxun.commons.generator.config.ConstVal;
import com.zhongxun.commons.generator.config.DataSourceConfig;
import com.zhongxun.commons.generator.config.GlobalConfig;
import com.zhongxun.commons.generator.config.PackageConfig;
import com.zhongxun.commons.generator.config.StrategyConfig;
import com.zhongxun.commons.generator.config.TemplateConfig;
import com.zhongxun.commons.generator.config.po.TableField;
import com.zhongxun.commons.generator.config.po.TableInfo;
import com.zhongxun.commons.generator.config.rules.DbType;
import com.zhongxun.commons.generator.config.rules.NamingStrategy;
import com.zhongxun.commons.generator.config.rules.QuerySQL;
...
public class ConfigBuilder {
	...
	private void handlerPackage(String outputDir, PackageConfig config) {
		...
		pathInfo.put(ConstVal.LIST_PATH, joinPath(outputDir, "jsp")); 
		pathInfo.put(ConstVal.ADD_PATH, joinPath(outputDir, "jsp")); 
		pathInfo.put(ConstVal.EDIT_PATH, joinPath(outputDir, "jsp")); 
	}
	...
	private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, String tablePrefix) {
		for (TableInfo tableInfo : tableList) {
			...
			if (StringUtils.isNotEmpty(globalConfig.getListName())) {
				tableInfo.setListName(String.format(globalConfig.getListName(), tableInfo.getEntityName()));
			} else {
				tableInfo.setListName(tableInfo.getEntityName() + ConstVal.LIST);
			}
			tableInfo.setListName(toLowerCaseFirstOne(tableInfo.getListName()));
			
			if (StringUtils.isNotEmpty(globalConfig.getAddName())) {
				tableInfo.setAddName(String.format(globalConfig.getAddName(), tableInfo.getEntityName()));
			} else {
				tableInfo.setAddName(tableInfo.getEntityName() + ConstVal.ADD);
			}
			tableInfo.setAddName(toLowerCaseFirstOne(tableInfo.getAddName()));
			
			if (StringUtils.isNotEmpty(globalConfig.getEditName())) {
				tableInfo.setEditName(String.format(globalConfig.getEditName(), tableInfo.getEntityName()));
			} else {
				tableInfo.setEditName(tableInfo.getEntityName() + ConstVal.EDIT);
			}
			tableInfo.setEditName(toLowerCaseFirstOne(tableInfo.getEditName()));
		}
		return tableList;
	}
